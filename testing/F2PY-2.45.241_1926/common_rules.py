...
$Date: 2004/09/15 17:45:41 $
Pearu Peterson
"""

__version__ = "$Revision: 1.17 $"[10:-1]
...
def buildhooks(m):
    ...
    for (name,vnames,vars) in findcommonblocks(m):
        ...
        cadd('static void f2py_setup_%s(%s) {'%(name,string.join(\
            map(lambda s:'char *'+s,inames1),',')))
        cadd('\tint i_f2py=0;')
        for n in inames1:
            cadd('\tf2py_%s_def[i_f2py++].data = %s;'%(name,n))
        cadd('}')
        cadd('extern void F_FUNC(f2pyinit%s,F2PYINIT%s)();'%(lower_name,string.upper(name)))
        cadd('static void f2py_init_%s() {'%name)
        cadd('\tF_FUNC(f2pyinit%s,F2PYINIT%s)(f2py_setup_%s);'%(lower_name,string.upper(name),name))
        cadd('}\n')
