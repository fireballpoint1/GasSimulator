...
$Date: 2004/12/04 10:08:56 $
Pearu Peterson
"""
__version__ = "$Revision: 1.161 $"[10:-1]
...
"""
  ...
  *** it is a list of dictionaries containing `blocks':
      B = {'block','body','vars'[,'name','prefix','args','result',
...
def readfortrancode(ffile,dowithline=show,istop=1):
    ...
    while 1:
        ...
        if sourcecodeform=='fix':
            ...
            if len(l)>5 and not l[5]==' ':                # Continuation of a previous line
                ll=ll+l[6:]
                finalline=''
                origfinalline=''
            else:
        ...
        if groupcache[groupcounter].has_key('externals'):
            for name in groupcache[groupcounter]['externals']:
                ...
                if m1:
                    ...
                    if m2:
                        line='callfun %s(%s) result (%s)'%(name,m1.group('args'),m2.group('result'))
                    else: line='callfun %s(%s)'%(name,m1.group('args'))
...
def get_useparameters(block, param_map=None):
    ...
    if param_map is None:
        param_map = {}
    if not block.has_key('use'):
        return param_map
    for usename,mapping in block['use'].items():
            ...
            if vars[n].has_key('dimension'):
                ...
                if vars[n].has_key('charselector'):
                    if vars[n]['charselector'].has_key('*'):
                        length=vars[n]['charselector']['*']
                        if length in ['(:)','(*)','*']:
                            length='(*)'
                        else:
                            try:
                                length = str(eval(length,{},params))
                            except Exception,msg:
                                errmess('"%s" in evaluating %r '\
                                        '(available names: %s)\n' \
                                        % (msg,length,params.keys()))
                        vars[n]['charselector']['*']=length
                    elif vars[n]['charselector'].has_key('len'):
                        length=vars[n]['charselector']['len']
                        if length in ['(:)',':','*']:
                            length='(*)'                            
                        else:
                            try:
            ...
            if block['block']=='function':
                ...
                if block.has_key('prefix'):
                    ...
                    if m:
                        ...
                        if kindselect: vars[n]['kindselector']=kindselect
