...
$Date: 2004/09/01 08:27:59 $
Pearu Peterson
"""

__version__ = "$Revision: 1.26 $"[10:-1]
...
def buildhooks(pymod):
    ...
    for m in findf90modules(pymod):
        ...
        iadd('extern void F_FUNC(f2pyinit%s,F2PYINIT%s)(void (*)(%s));'%(m['name'],string.upper(m['name']),string.join(sargsp,',')))
        iadd('static void f2py_init_%s(void) {'%(m['name']))
        iadd('\tF_FUNC(f2pyinit%s,F2PYINIT%s)(f2py_setup_%s);'%(m['name'],string.upper(m['name']),m['name']))
        iadd('}\n')
