...
def MIXERC():
	...
	#COMMON/MIXC/
	global PRSH,ESH,AUG,RAD,PRSHBT,IZ,INIOCC,ISHLMX,AMZ 
	#COMMON/MIXPE/
	global XPE,YPE
	#COMMON/MIXCN/
	global XCP,YRY,YCP,YPP
	#COMMON/COMPTIN/
	global FRMFR,FRMFC
	#COMMON/GASN/
	global NGASN
			...
			for J in range(1,17+1):
				...
				for K in range(1,17+1):
					...
					for L in range(1,17+1):
						AUG[I][M][J][K][L]=0.0
	GASMIXC(NGASN[1],PRSH1,PRSHBT1,ESH1,AUG1,RAD1,XP1,YP1,XC1,YR1,YC1,YPP1,FFAR1,FFAC1,IZ1,AMZ1,INIOCC1)
	if(NGAS == 1):
		pass
	else:
		GASMIXC(NGASN[2],PRSH2,PRSHBT2,ESH2,AUG2,RAD2,XP2,YP2,XC2,YR2,YC2,YPP2,FFAR2,FFAC2,IZ2,AMZ2,INIOCC2)
		if(NGAS == 2):
			pass
		else:
			GASMIXC(NGASN(3),PRSH3,PRSHBT3,ESH3,AUG3,RAD3,XP3,YP3,XC3,YR3,YC3,YPP3,FFAR3,FFAC3,IZ3,AMZ3,INIOCC3)
			if(NGAS == 3):
				pass
			else:
				GASMIXC(NGASN[4],PRSH4,PRSHBT4,ESH4,AUG4,RAD4,XP4,YP4,XC4,YR4,YC4,YPP4,FFAR4,FFAC4,IZ4,AMZ4,INIOCC4)
				if(NGAS == 4):
					pass
				else:
					GASMIXC(NGASN[5],PRSH5,PRSHBT5,ESH5,AUG5,RAD5,XP5,YP5,XC5,YR5,YC5,YPP5,FFAR5,FFAC5,IZ5,AMZ5,INIOCC5)
					if(NGAS == 5):
						pass
					else:
						GASMIXC(NGASN[6],PRSH6,PRSHBT6,ESH6,AUG6,RAD6,XP6,YP6,XC6,YR6,YC6,YPP6,FFAR6,FFAC6,IZ6,AMZ6,INIOCC6)
						if(NGAS == 6):
							pass
	# 10 CONTINUE
	I=1
	...
	for J in range(1,3+1):
		for K in range(1,45+1):
			...
			FRMFC[6][J][K]=FFAC6[J][K]
	if(NGAS > 6):
		print(' subroutine STOPPED: NGAS=',NGAS,' IN MIXERC')
		sys.exit()
	# endif
	# 1000 CONTINUE
	return
